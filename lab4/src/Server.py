...
class Server_API:
    def __init__(self):
        ...
        self.conn = sqlite3.connect(
            self.serverdb_path, check_same_thread=False)
    ...
    def register(self, username, password):
        if EnableCS:
            ...
            try:
                ...
                self.conn.commit()
                print("Sign up: username = %s, password = %s" %
                      (username, password))
                return True
            finally:
                self.lock.release()
    ...
    def subject(self):
        ...
        df = pd.DataFrame(
            table_rows,
            columns=["topic_name", "description",
                     "founder_name", "founded_time"],
        )
        ...
        if EnableCS:
            ...
            try:
                ...
                if not self.c.fetchone():
                    print("Topic: %s does not exist" % (topic_name))
                    return False
                self.c.execute(f"SELECT COUNT(*) FROM {topic_name}")
                len = self.c.fetchone()[0]
                if len <= 1:
                    self.c.execute(
                        "DELETE FROM TOPIC WHERE topic_name = '%s'" % topic_name)
                    ...
                    print(f"{topic_name} Topic has {len} replies")
                    return False
            finally:
                self.lock.release()
